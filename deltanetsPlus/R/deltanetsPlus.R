...
#' @param cv.method "cv"(cross-validation, default) or "loo"(leave-one-out). If "loo" is selected, the input of \code{kfolds} will be ignored.
...
#' @param numClusters The number of cores for parallel computing. The default is 4.
#'
...
#'
#' @export
deltanetsPlus <- function(data, slope=NULL, grn=NULL, perturbation=c("group","individual"), group=NULL, cv.method=c("cv","loo"), kfolds=10, lambda=NULL, cv.opt = c("lambda.min","lambda.1se"), gset=NULL, A.return=FALSE, par=FALSE, numClusters=4){
  ...
  if(!is.element(cv.method,c("cv","loo"))){
    stop("Choose either'cv' or 'loo' in cv.method.")
  }
  ...
  if(par){
    ...
    Sys.sleep(3)
    Beta <- foreach(j=1:length(dgi),.combine = cbind,.packages = "glmnet", .options.snow=opts) %dopar% {
      # pb$tick(1)
      Sys.sleep(1 / 100)
